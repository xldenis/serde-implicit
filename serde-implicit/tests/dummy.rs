...
fn tuple_basic() {
    ...
    enum TupleEnum {
        Case1(bool, u32),
        Case2(u32, #[serde_implicit(tag)] String),
        Case3(u32, #[serde_implicit(tag)] bool),
    }

    let res: Result<TupleEnum, _> = serde_json::from_value(json!([true, 0]));
    assert!(res.is_ok(), "{res:?}");

    let res: Result<TupleEnum, _> = serde_json::from_value(json!([0, "omg"]));

    assert!(res.is_ok(), "{res:?}");

    let res: Result<TupleEnum, _> = serde_json::from_value(json!([0, true]));
